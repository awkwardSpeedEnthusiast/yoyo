...
auto const communication_root_name = "yoyo::communication_root"s;
auto const message_container_name = "yoyo::message_container"s;
auto const message_name = "yoyo::message"s;
auto const message_field_name = "yoyo::message_field"s;

constexpr boost::uuids::uuid const communication_root_id = {
  { 0x02, 0xb1, 0x79, 0x7a, 0x6e, 0x71, 0x45, 0x13, 0xa4, 0x43, 0x25, 0x24, 0x5b, 0x75, 0xe0, 0xa9 }
};
constexpr boost::uuids::uuid const message_container_id = {
  { 0x95, 0x18, 0xae, 0x42, 0xbd, 0xdd, 0x4d, 0x55, 0xba, 0x07, 0x6c, 0xcb, 0x68, 0xe2, 0xfc, 0xf4 }
};
constexpr boost::uuids::uuid const message_id = {
  { 0x1f, 0x07, 0x73, 0xbc, 0x68, 0x5e, 0x4d, 0xc0, 0x88, 0xc4, 0xb0, 0x59, 0x95, 0x19, 0xc5, 0xa0 }
};
...
auto contextmenu_handler::execute(std::shared_ptr<node_base> current, QPoint p) -> void
{
...

  {
    if (auto parent = current->parent().lock()) {
      ...
      } else if (parent->staticTypeId() == message_container_id
                 || parent->staticTypeId() == message_id) {
        ::addActionsToSibling(beforeMenu, afterMenu, parent, parent->childIndex(current),
                              _factories[0], _factories[0]);
      } else if (parent->metaObject()->inherits(&communication_node::staticMetaObject)
                 || parent->metaObject()->inherits(&communication_node_base::staticMetaObject)) {
        ::addActionsToSibling(beforeMenu, afterMenu, parent, parent->childIndex(current),
                              _factories[4], _factories[4]);
      }
    }
  }
                                                  ...
                                                  bool connectDefaultAutoConnect) -> void
{
  ...
  auto node_id = boost::uuids::uuid { { 0x81, 0xc2, 0xdf, 0x83, 0x24, 0x52, 0x42, 0x2c, 0x88, 0x46,
                                        0xd8, 0xbf, 0xa2, 0xd7, 0x56, 0xdf } };
