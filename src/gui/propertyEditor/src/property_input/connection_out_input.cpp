...
namespace yoyo::gui
{
connection_out_input::connection_out_input(std::shared_ptr<node_base> node,
  ...
  , _input { std::make_unique<QLineEdit>(this) }
{
  ...
  connect(_input.get(), &QLineEdit::returnPressed, this, [this, propertyName]() {
    _buffer._out = _input->text();
    Q_EMIT propertyChanged(propertyName, QVariant::fromValue(_buffer));
  });

  onPropertyChanged(node->property(propertyName.c_str()).value<properties::out_connection_t>());
}

connection_out_input::~connection_out_input() = default;

auto connection_out_input::onPropertyChanged(properties::out_connection_t value) -> void
{
  _buffer = value;
  _input->setText(_buffer._out);
}
} // namespace yoyo::gui
